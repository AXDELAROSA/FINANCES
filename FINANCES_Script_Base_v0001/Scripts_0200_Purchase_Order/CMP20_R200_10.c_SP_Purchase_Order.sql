...
-- EXECUTE [dbo].[PG_LI_HEADER_PURCHASE_ORDER] 0,131, '' , -1 , -1 , -1 , '2021-09-01' , '2021-09-30'
...
BEGIN
	...
	AND			( @PP_F_FINISH IS NULL		OR	@PP_F_FINISH>=F_DATE_PURCHASE_ORDER)
				-- =============================
	AND			( HEADER_PURCHASE_ORDER.K_STATUS_PURCHASE_ORDER IN (4,5,6,7,8,9,11,12) )
	AND			( @PP_K_VENDOR =-1			OR	HEADER_PURCHASE_ORDER.K_VENDOR=@PP_K_VENDOR )
	AND			( @PP_K_CURRENCY =-1		OR	HEADER_PURCHASE_ORDER.K_CURRENCY=@PP_K_CURRENCY )
	AND			( @VP_K_APPROVED_BY =-1		OR	HEADER_PURCHASE_ORDER.K_APPROVED_BY=@VP_K_APPROVED_BY )
				-- =============================
...
CREATE PROCEDURE [dbo].[PG_IN_HEADER_PURCHASE_ORDER]
	...
	-----=====================================================
	@PP_L_RETENTION							[INT]			= 0,
	@PP_ISR_RETEN							[DECIMAL](10,4)	= 0,
	@PP_IVA_RETEN							[DECIMAL](10,4)	= 0
AS			
DECLARE  @VP_MENSAJE						VARCHAR(500)	= ''
		,@VP_K_HEADER_PURCHASE_ORDER		INT				= 0
		,@PP_K_PO_TEMPORAL					INT				= 0
		,@VP_K_STATUS						INT				= 1
...
BEGIN TRY
	...
	END
		...
		SET		
				...
				-- ============================		= -- ============================
				[L_RETENTION]						= @PP_L_RETENTION,
				[ISR_RETEN]							= @PP_ISR_RETEN,
				[IVA_RETEN]							= @PP_IVA_RETEN,
				-- ============================		= -- ============================
				[F_CAMBIO]							= GETDATE(), 
...
-- //////////////////////////////////////////////////////////////
-- // STORED PROCEDURE ---> UPDATE / FICHA
-- // SP PARA ACTUALIZAR UNA ORDEN DE COMPRA CUÁNDO YA FUE APROBADA
...
BEGIN TRY
	...
	BEGIN
		SET @VP_ESTATUS_PO_DEPTS	= 4
	END
	ELSE IF	@VP_ESTATUS_PO_DEPTS IN (6,7,8,9,11,12)
	BEGIN
...
BEGIN TRY
	...
	BEGIN
		...
		EXECUTE [PG_PR_ENVIAR_CORREO_CANCELADA_RECHAZADA]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION, @PP_K_HEADER_PURCHASE_ORDER,	@PP_K_PO_TEMPORAL, 1
	END
	ELSE
	BEGIN
...
BEGIN TRY
	...
	WHILE patindex('%/%' , @PP_PO_ARRAY) <> 0
		BEGIN
			...
			BEGIN
				EXECUTE [PG_PR_ENVIAR_CORREO_CANCELADA_RECHAZADA]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION, @VP_PO_INT, @VP_VALOR_TM,	0
			END
...
--		DECLARE @VP_PO_INT INT;
		...
		--END
			--SET @VP_FILE_PATH = '\\10.1.1.5\documents\Common\COMPRAS\REPORTES\PO_'+ CONVERT(VARCHAR(10),FORMAT(@PP_VALOR_PO,'000000')) +'.PDF'  		
			SET @VP_SUBJECT = 'PEARL LEATHER CANC-RECH[PO#' + CONVERT(VARCHAR(10),FORMAT(@PP_VALOR_PO,'000000')) +'//'+ CONVERT(VARCHAR(10),FORMAT(@PP_VALOR_TEMPORAL,'000000'))+']'--CONVERT(VARCHAR(10),FORMAT(@VP_VALOR_PO,'000000'))+']'					
