...
-- CAMBIO_FIRMA			--WHEN	EN_NUM_DEPT IN (7,12)		THEN	(181)			-- PROYECTOS			OMARD
	...
	BEGIN
		--SET @VP_LI_N_REGISTROS = 0		
			...
			INNER JOIN	STATUS_PAID_ORDER		ON STATUS_PAID_ORDER.K_STATUS_PAID_ORDER=HEADER_PURCHASE_ORDER.K_STATUS_PAID_ORDER
						-- =============================
			...
			AND			( @VP_K_APPROVED_BY =-1		OR	HEADER_PURCHASE_ORDER.K_APPROVED_BY=@VP_K_APPROVED_BY )
						-- =============================
---			AND			K_HEADER_PURCHASE_ORDER IN (30,35,60,76,77,167,168,169)
						-- =============================
			AND			HEADER_PURCHASE_ORDER.L_BORRADO<>1
			--ORDER BY	K_STATUS_PURCHASE_ORDER, F_DATE_PURCHASE_ORDER	DESC
		UNION
			SELECT		CONVERT(DECIMAL(10,2),(TAX_RATE*100)) AS TAX,
						-- =============================	 
						F_DATE_PURCHASE_ORDER AS [DATE],
						F_REQUIRED_PURCHASE_ORDER AS [REQUIRED],
						F_PROMISE_PURCHASE_ORDER AS  [PROMISE],
			...
			INNER JOIN	STATUS_PAID_ORDER		ON STATUS_PAID_ORDER.K_STATUS_PAID_ORDER=HEADER_PURCHASE_ORDER.K_STATUS_PAID_ORDER
						-- =============================
			WHERE		K_HEADER_PURCHASE_ORDER IN (30,35,60,76,77,167,168,169)
						-- =============================
			AND			HEADER_PURCHASE_ORDER.L_BORRADO<>1
			ORDER BY	K_STATUS_PURCHASE_ORDER, F_DATE_PURCHASE_ORDER	DESC
	END
...
GO
-- EXECUTE [dbo].[PG_SK_HEADER_PURCHASE_ORDER] 0,139,0,6
-- EXECUTE [dbo].[PG_SK_HEADER_PURCHASE_ORDER] 0,139,32
-- EXECUTE [dbo].[PG_SK_HEADER_PURCHASE_ORDER] 0,139,7
-- EXECUTE [dbo].[PG_SK_HEADER_PURCHASE_ORDER] 0,139,78,0
...
GO
-- EXECUTE [dbo].[PG_SK_DETAILS_PURCHASE_ORDER] 0,139,22,0
-- EXECUTE [PG_SK_DETAILS_PURCHASE_ORDER] 0,139,  0 , 11 
...
GO
-- EXECUTE	[PG_SK_DETAILS_LOG] 0,139,  22,0
-- EXECUTE	[PG_SK_DETAILS_LOG] 0,139,  125,0
...
AS
	...
	-- ///////////////////////////////////////////
		...
		FROM		LOG_DETAILS_PURCHASE_ORDER
		INNER JOIN	ITEM		ON	LOG_DETAILS_PURCHASE_ORDER.K_ITEM=ITEM.K_ITEM
		INNER JOIN	BD_GENERAL.DBO.UNIT_OF_MEASURE ON ITEM.K_UNIT_OF_ITEM=UNIT_OF_MEASURE.K_UNIT_OF_MEASURE
		INNER JOIN	BD_GENERAL.DBO.CURRENCY	ON	ITEM.K_CURRENCY=CURRENCY.K_CURRENCY
		---INNER JOIN	LOG_DETAILS_PURCHASE_ORDER	ON HEADER_PURCHASE_ORDER.K_HEADER_PURCHASE_ORDER=LOG_DETAILS_PURCHASE_ORDER.K_HEADER_PURCHASE_ORDER
...
-- CAMBIO_FIRMA		--WHEN	EN_NUM_DEPT IN (2,6,11)		THEN	(SELECT CONCAT(EP_NOMBRE,' ',EP_APELLIDO_PATERNO) FROM	HOWE.DBO.VISTA_GAFETES WHERE EN_NUM_DEPT=6 AND 	EN_SUPERVISOR='GERENTES')						-- PRODUCCION
-- CAMBIO_FIRMA		--WHEN	EN_NUM_DEPT IN (5)			THEN	(SELECT CONCAT(EP_NOMBRE,' ',EP_APELLIDO_PATERNO) FROM	HOWE.DBO.VISTA_GAFETES WHERE EN_NUM_DEPT=5 AND 	EN_SUPERVISOR='GERENTES')						-- MANTENIMIENTO
...
-- CAMBIO_FIRMA		--WHEN	EN_NUM_DEPT IN (7,12)		THEN	(SELECT CONCAT(EP_NOMBRE,' ',EP_APELLIDO_PATERNO) FROM	HOWE.DBO.VISTA_GAFETES WHERE EN_NUM_DEPT=12 AND EN_SUPERVISOR='GERENTES')					-- PROYECTOS / SISTEMAS
					WHEN	EN_NUM_DEPT IN (12)			THEN	(SELECT CONCAT(EP_NOMBRE,' ',EP_APELLIDO_PATERNO) FROM	HOWE.DBO.VISTA_GAFETES WHERE EN_NUM_DEPT=12 AND EN_SUPERVISOR='GERENTES')					-- PROYECTOS	-- CAMBIO_FIRMA		--
					WHEN	EN_NUM_DEPT IN (7)			THEN	(SELECT CONCAT(EP_NOMBRE,' ',EP_APELLIDO_PATERNO) FROM	HOWE.DBO.VISTA_GAFETES WHERE EN_NUM_DEPT=7 AND PT_PUESTO_TRABAJO='GSI')					-- SISTEMAS		-- CAMBIO_FIRMA		--
...
-- CAMBIO_FIRMA		--IF @PP_K_USUARIO_ACCION IN (62,44,56,47,42,60,43,44,156)			--CON OMAR Y CON RAFA
	...
	BEGIN
		...
		IF @@ROWCOUNT = 0
			BEGIN
				--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
				SET @VP_MENSAJE='The PO_TEMP was not updated. [HEADER_PURCHASE_ORDER_TEMP#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+']'
				RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
			END			
...
BEGIN TRY
	...
	--IF @VP_K_ESTATUS < 6
	--BEGIN
	IF @PP_K_USUARIO_ACCION NOT IN (57,139,69,157,60)
	BEGIN
		UPDATE	HEADER_PURCHASE_ORDER
			SET	
				[C_PURCHASE_ORDER]					= @PP_C_PURCHASE_ORDER,
		...
		IF @@ROWCOUNT = 0
			BEGIN
				SET @VP_MENSAJE='The [TAX_RATE] value was not updated. [HEADER_PURCHASE_ORDER#'+CONVERT(VARCHAR(10),@PP_K_HEADER_PURCHASE_ORDER)+']'
				RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
			END
	END
	--END
	--ELSE
	--BEGIN
	--	SET @VP_MENSAJE='Status not valid for this action, verify.'
	--	RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
	--END
...
BEGIN TRY
	...
	BEGIN
		...
		IF @@ROWCOUNT = 0
			BEGIN
				SET @VP_MENSAJE='The STATUS was not changed. [HEADER_PURCHASE_ORDER#'+CONVERT(VARCHAR(10),@PP_K_HEADER_PURCHASE_ORDER)+']'
				RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
			END
	END
	ELSE
	BEGIN
		SET @VP_MENSAJE='The PURCHASE_ORDER it was already canceled'
	END
-- /////////////////////////////////////////////////////////////////////
COMMIT TRANSACTION 
END TRY
		...
		BEGIN
			...
			DECLARE @VP_TOTAL_RECIBIDA	DECIMAL = 0
					...
					IF @@ROWCOUNT = 0
						BEGIN
							--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
							SET @VP_MENSAJE='The LOG_DETAIL_PURCHASE_ORDER was not updated. [DETAIL#'+CONVERT(VARCHAR(10),@VP_VALOR_ITEM)+']'
							RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
						END
...
--		AND		K_STATUS_PURCHASE_ORDER IN (6,7)
		...
		IF @@ROWCOUNT = 0
			BEGIN
				--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
				SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] NOT AVAILABLE'
				RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
			END			
	END
	...
	IF @VP_MENSAJE=''
	BEGIN
		IF	@VP_ESTATUS_PO_ORIGI NOT IN (6,7)
		BEGIN
			SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] NOT AVAILABLE FOR PRINT'
			RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
		END
		ELSE
		BEGIN
				...
				----================================================================
					...
					AND		[K_PO_TEMPORAL]=	@PP_K_PO_TEMPORAL
						...
						IF @@ROWCOUNT = 0
							BEGIN
								--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
								SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] Not UPDATE ESTATUS_PRINT & PO<>0'
								RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
							END
				...
				SET @PP_VP_K_LIVE = @PP_VP_ULTIMO_REGISTRO + 1
					...
					AND		[K_PO_TEMPORAL]=@PP_K_PO_TEMPORAL
						...
						IF @@ROWCOUNT = 0
							BEGIN
								--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
								SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] Not UPDATE STATUS'
								RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
							END
					----================================================================
					----================================================================
					...
					AND		[K_PO_TEMPORAL]=@PP_K_PO_TEMPORAL
						...
						IF @@ROWCOUNT = 0
							BEGIN
								--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
								SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] Not UPDATE DETAILS'
								RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
							END
					----================================================================
					----================================================================
					...
					BEGIN
						...
						AND		[K_PO_TEMPORAL]=@PP_K_PO_TEMPORAL
							...
							IF @@ROWCOUNT = 0
								BEGIN
									--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
									SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] Not UPDATE DETAILS BLANKET'
									RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
								END
					END
					----================================================================
					...
					WHERE [K_HEADER_PURCHASE_ORDER_TEMP]=@PP_K_PO_TEMPORAL
						...
						IF @@ROWCOUNT = 0
							BEGIN
								--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
								SET @VP_MENSAJE='[PO#'+CONVERT(VARCHAR(10),@PP_K_PO_TEMPORAL)+'] Not UPDATE table TEMP.'
								RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
							END		
					----================================================================		
