...
AS
	...
	DECLARE @VP_COLOR_PEARL VARCHAR(50) = ''
	--SELECT	@VP_COLOR_PEARL = RTRIM(LTRIM(ITEM_NO))
	--FROM	IMITMIDX_SQL 
	--WHERE	RTRIM(LTRIM(search_desc)) = @PP_TYPE
	SELECT	TOP(1) @VP_COLOR_PEARL = PCOLOR
		FROM	HIDESHDR_SQL
		WHERE	
		--dbo.CONVERT_INT_TO_DATE(CONVERT(INT,TDATE)) >= @PP_F_INICIO 
		--AND		dbo.CONVERT_INT_TO_DATE(CONVERT(INT,TDATE)) <= @PP_F_FIN
		--LTRIM(RTRIM(PCOLOR)) = @VP_COLOR_PEARL
		LTRIM(RTRIM(TYPE)) = @PP_TYPE
