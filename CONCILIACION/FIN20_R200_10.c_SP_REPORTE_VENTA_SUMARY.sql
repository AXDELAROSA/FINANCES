...
AS
	...
	SET @VP_QUERY = N'SELECT  ''          ''  TOTAL_PRICE_COMPLETED, ''          ''  TOTAL_PRICE_PER_PIECE, ''          ''  CUTTING_COST, ''          ''  EMBOSSING, ''          ''  LAMINATION, ''          ''  PERFORATION, ''          ''  QUILTING, ''          ''  RECUT, ''          ''  SKIVING, ''''  COLOR,  ''''  PART_NO, ''INVOICE DATE''  CUS_PART_NO, '''' NET_AREA, '''' GROSS, '''' PRECIO, '''' ACUMULADO_QTY, '''' ACUMULADO_MOUNT, '''' CUTTING_MOUNT, ' + @VP_COLUMNA + N' INTO [tempdb].[dbo].[FECHA_INVOICE]  FROM ( SELECT  FECHA_INVOICE, INVOICE FROM #MATERIAL_VENDIDO GROUP BY FECHA_INVOICE, INVOICE ) x pivot ( MAX(FECHA_INVOICE) for INVOICE in (' + @VP_COLUMNA + N') ) p ' 
	EXEC sp_executesql @VP_QUERY;	

	SET @VP_QUERY = ''
	SET @VP_QUERY = N'SELECT ''          ''  TOTAL_PRICE_COMPLETED, ''          ''  TOTAL_PRICE_PER_PIECE, ''          ''  CUTTING_COST, ''          ''  EMBOSSING, ''          ''  LAMINATION, ''          ''  PERFORATION, ''          ''  QUILTING, ''          ''  RECUT, ''          ''  SKIVING, ''''  COLOR,  '''+ @PP_D_COLOR +'''  PART_NO, ''PACKING #''  CUS_PART_NO, '''' NET_AREA, '''' GROSS, '''' PRECIO, '''' ACUMULADO_QTY, '''' ACUMULADO_MOUNT, '''' CUTTING_MOUNT, ' + @VP_COLUMNA + N' INTO [tempdb].[dbo].[PACKING_INVOICE]  FROM ( SELECT  PACKING, INVOICE FROM #MATERIAL_VENDIDO GROUP BY PACKING, INVOICE ) x pivot ( MAX(PACKING) for INVOICE in (' + @VP_COLUMNA + N') ) p ' 
	EXEC sp_executesql @VP_QUERY;	

	SET @VP_QUERY = ''
	SET @VP_QUERY = N'SELECT ''          ''  TOTAL_PRICE_COMPLETED, ''          ''  TOTAL_PRICE_PER_PIECE, ''          ''  CUTTING_COST, ''          ''  EMBOSSING, ''          ''  LAMINATION, ''          ''  PERFORATION, ''          ''  QUILTING, ''          ''  RECUT, ''          ''  SKIVING, ''''  COLOR,  ''''  PART_NO, ''INVOICE #''  CUS_PART_NO, '''' NET_AREA, '''' GROSS, '''' PRECIO, '''' ACUMULADO_QTY, '''' ACUMULADO_MOUNT, '''' CUTTING_MOUNT, ' + @VP_COLUMNA + N' INTO [tempdb].[dbo].[FACTURAS]  FROM ( SELECT  INVOICE FROM #MATERIAL_VENDIDO GROUP BY  INVOICE ) x pivot ( MAX(INVOICE) for INVOICE in (' + @VP_COLUMNA + N') ) p ' 
	EXEC sp_executesql @VP_QUERY;	

	SET @VP_QUERY = ''
	SET @VP_QUERY = N'SELECT ''          ''  TOTAL_PRICE_COMPLETED, ''          ''  TOTAL_PRICE_PER_PIECE, ''          ''  CUTTING_COST, ''          ''  EMBOSSING, ''          ''  LAMINATION, ''          ''  PERFORATION, ''          ''  QUILTING, ''          ''  RECUT, ''          ''  SKIVING, ''''  COLOR, ''''  PART_NO, ''TOTAL USD $''  CUS_PART_NO, '''' NET_AREA, '''' GROSS, '''' PRECIO, '''' ACUMULADO_QTY, '''' ACUMULADO_MOUNT, '''' CUTTING_MOUNT, ' + @VP_COLUMNA + N' INTO [tempdb].[dbo].[TOTAL_INVOICE]  FROM ( SELECT  SUM(CONVERT(DECIMAL(13,2), MOUNT)) AS TOTAL_INVOICE, INVOICE FROM #MATERIAL_VENDIDO GROUP BY INVOICE ) x pivot ( MAX(TOTAL_INVOICE) for INVOICE in (' + @VP_COLUMNA + N') ) p ' 
	EXEC sp_executesql @VP_QUERY;	

	SET @VP_QUERY = ''
	SET @VP_QUERY = N'SELECT ''          ''  TOTAL_PRICE_COMPLETED, ''          ''  TOTAL_PRICE_PER_PIECE, ''          ''  CUTTING_COST, ''          ''  EMBOSSING, ''          ''  LAMINATION, ''          ''  PERFORATION, ''          ''  QUILTING, ''          ''  RECUT, ''          ''  SKIVING, COLOR, PART_NO, CUS_PART_NO, NET_AREA, GROSS, PRECIO, ''          '' ACUMULADO_QTY,  ''          '' ACUMULADO_MOUNT, ''          '' CUTTING_MOUNT, ' + @VP_COLUMNA + N' INTO [tempdb].[dbo].[QTY_SHIP]  FROM ( SELECT COLOR, PART_NO, CUS_PART_NO, NET_AREA, GROSS, PRECIO, ACUMULADO_QTY, ACUMULADO_MOUNT, QTY, INVOICE FROM #MATERIAL_VENDIDO ) x pivot ( MAX(QTY) for INVOICE in (' + @VP_COLUMNA + N') ) p ' 
	EXEC sp_executesql @VP_QUERY;	
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN		
			...
			--	SET @VP_YIELD = (@VP_NET_AREA / @VP_UTILIZACION) * 100
				--SET @VP_YIELD = ( @VP_NET_AREA / @VP_UTILIZACION * 100 ) * @VP_ACUMULADO_QTY
			...
			DECLARE @OU_TOTAL_NET_AREA VARCHAR(50), @OU_TOTAL_GROSS_AREA VARCHAR(50);
			...
			UPDATE [tempdb].[dbo].[VENTA_TOTAL] 
				SET 
					--GROSS			= CONVERT(VARCHAR(15), @VP_YIELD),
